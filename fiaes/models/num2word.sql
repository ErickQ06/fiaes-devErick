CREATE OR REPLACE FUNCTION public.f_num2str(num numeric)
...
DECLARE
     ...
     lnSw INTEGER;
BEGIN
     lnEntero := FLOOR(num)::INTEGER;--Obtenemos la parte Entera
     lnFraccion := FLOOR(((num - lnEntero) * 100))::INTEGER;--Obtenemos la Fraccion del Monto
     lcRetorno := '';
     lnTerna := 1;
     IF lnEntero > 0 THEN
...
--
CREATE OR REPLACE FUNCTION public.triggerupdate()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$ 
        begin
           new.x_letras = f_num2str(new.amount);
          return new;
        END;
    $function$
;
...
set x_letras = f_num2str(account_payment.amount) 

--
 CREATE TRIGGER num2word
  before insert or update
  ON account_payment
  FOR EACH ROW
  EXECUTE PROCEDURE triggerupdate();
